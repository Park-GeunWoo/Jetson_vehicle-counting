...
from data.data_store import count, data_store_init
...
class YOLOTracker:
    def __init__(self, args):
        ...
        self.input_path = 'input남측18.mp4' #input 영상 이름
        ...
        #self.model.export(format="engine", device='dla:0', dynamic=True, batch=8, workspace=4, half=True)
        #elf.model = YOLO('yolov8n.engine',task='detect')
    ...
    def run(self):
        ...
        while True:
            ...
            index += 1
        end_time=time.time()
        print(f'Total Frames: {index} Total Time: {int(end_time-start_time)} Average FPS: {int(avrg_fps/index)}')
